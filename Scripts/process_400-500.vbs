...
Dim dictAltUserName1: Set dictAltUserName1 = CreateObject("Scripting.Dictionary")
Dim dictAltUserName2: Set dictAltUserName2 = CreateObject("Scripting.Dictionary")
...
Dim intPortloc
Dim tmpLogItems
Dim internalIP: internalIP = "172.16.36.42"
 strFile= "C:\ADFS\XXX_adfs_01.txt"
 strIDPath = "C:\ADFS\XXX_adfs_01.log"
 StrOutfile = "C:\ADFS\XXX_adfs_01.CSV"
    ...
    if instr(strData, "TimeCreated  : ") > 0 then
      ...
      for each strEvent in ArrayEvents
        if instr(strEvent, vbcrlf) then
          ...
          strRely = getdata(strEvent, vbcrlf ,"Relying party: ")
            ...
            for each IpAddress in arrayIPaddresses
              ...
              if instr(strIP, ",") then
                ...
                for each strTmpIP in arrayIP
                  if replace(strTmpIP, " ", "") <> internalIP then
                ...
                next
              elseif strIP <> "" and  replace(strIP, " ", "") <> internalIP and DictIP.exists(strInstanceID) = false then
                DictIP.add strInstanceID, replace(strIP, " ", "")
              elseif strIP <> "" and  replace(strIP, " ", "") <> internalIP and DictIP.exists(strInstanceID) = true then
          ...
          strUser = getdata(strEvent, vbCr ,"http://schemas.xmlsoap.org/ws/2005/05/identity/claims/upn " & vbcrlf) 'email address
          
          if strUser <> "" and dictAltUserName1.exists(strInstanceID) = false and strInstanceID <> "" then  dictAltUserName1.add strInstanceID, replace(strUser, " ", "")  

          strUser = getdata(strEvent, vbCr ,"http://schemas.xmlsoap.org/ws/2005/05/identity/claims/implicitupn " & vbcrlf) 'alt email
          if strUser <> "" and dictAltUserName2.exists(strInstanceID) = false and strInstanceID <> "" then  dictAltUserName2.add strInstanceID, replace(strUser, " ", "")  
          
          
          strUser = getdata(strEvent, vbCr ,"http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name " & vbcrlf) 'user ID
          ...
          if DictIP.exists(strInstanceID) and (dictUserName.exists(strInstanceID) or dictAltUserName1.exists(strInstanceID) or dictAltUserName2.exists(strInstanceID))  then
           if dictIP.item(strInstanceID) = "Internal" or (dictClientApp.exists(strInstanceID) and dictUserAgent.exists(strInstanceID) and (dictSucIssue.exists(strInstanceID) or dictFailIssue.exists(strInstanceID))) then
              ...
              'Date Time	Instance ID	Activity ID	IP Address	User Name	Email	Alternate Email	Application	User Agent	Relying Party	Success Auth	Fail Auth

              logdata StrOutfile, strDate & "," &  strInstanceID & "," & dictInstanceID(strInstanceID) & "," & replace(DictIP.item(strInstanceID), "               "  & internalIP & " |", "") & "," & dictUserName.item(strInstanceID) & "," &  dictAltUserName1.item(strInstanceID) & "," &  dictAltUserName2.item(strInstanceID) & "," & dictClientApp.item(strInstanceID) & "," & dictUserAgent.item(strInstanceID) & "," & dictRelyParty(strInstanceID) & "," & StrSuc & "," & strFail, false
            end if
          end if
