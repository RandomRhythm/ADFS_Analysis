...
Dim intPortloc
Dim tmpLogItems
 strFile= "C:\ADFS\54_adfs_01.txt"
 strIDPath = "C:\ADFS\54_adfs_01.log"
 StrOutfile = "C:\ADFS\54_adfs_01.CSV"
    ...
    if instr(strData, "TimeCreated  : ") > 0 then
      ...
      for each strEvent in ArrayEvents
        if instr(strEvent, vbcrlf) then
          ...
          strLastDate = getdata(strEvent, vbcr ,"LastBadPasswordAttempt: ")
          if strDate <> "" and strPwdCount <> "" then
            
            rowOut = replace( strDate & "," &  strUser & "," & strPwdCount & "," & strLastDate,"               ", "")
