...
Dim intPortloc
Dim tmpLogItems
 strFile= "C:\ADFS\411_adfs_02.txt"
 strIDPath = "C:\ADFS\411_adfs_02_all.csv"
 StrOutfile = "C:\ADFS\411_adfs_02.CSV"
  ...
  Set objFile = objFSO.OpenTextFile(strFile)
    ...
    if instr(strData, "TimeCreated  : ") > 0 then
      ...
      for each strEvent in ArrayEvents
        if instr(strEvent, vbcrlf)  > 0  then
         if instr(strEvent ,vbcr) > 0 then  strDate = left(strEvent, instr(strEvent ,vbcr) -1)
          strUser = getdata(strEvent, " ", "System.IdentityModel.Tokens.SecurityTokenValidationException: ")

          if strUser = "" then strUser = getdata(strEvent, vbcr, "System.IdentityModel.Tokens.SecurityTokenValidationException: ")
          if instr(strUser, vbcrf) > 0 then strUser = replace(strUser, vbcrlf, "")
          strClientIP = getdata(strEvent, vbcr ,"Client IP: " & vbcrlf)
          strClientIP = replace(strClientIP, " ", "")
          if instr(strClientIP,",") = 0 then
          ...
          'Date Time	Email	IP Address	IP Address 2	Bad Password	Locked	Expired Password	Disabled

          rowOut = replace( strDate & "," &  strUser & "," & strClientIP & "," & boolBadPass & "," & boolLocked & "," & boolExpired & "," & boolDisabled,"                ", "")
